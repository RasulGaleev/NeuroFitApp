...
workout_instruction = {
    ...
    "content": (
        ...
        "Ответ должен быть массивом (списком) упражнений, где каждое упражнение — словарь с:\n"
        "- названием упражнения (name)\n"
        "- количеством подходов (sets)\n"
        "- количеством повторений (reps)\n"
        "- кратким описанием техники выполнения (description)\n\n"
        "Пример формата:\n"
        "[\n"
        "  {\n"
        ...
        "  }\n"
        "]\n\n"
        "Отвечай **только JSON-списком**, без дополнительных комментариев и внешних обёрток."
    )
}
