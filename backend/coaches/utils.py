...
def get_system_message(user: CustomUser, task: Optional[str] = None) -> dict:
    ...
    if task == "nutrition":
        message = base_message + (
            ...
            "- Похудение: примерно 1500 ккал\n"
            "- Поддержание: примерно 2000 ккал\n"
            "- Масса: примерно 3000 ккал\n\n"
            "Формат:\n"
        ...
        message = base_message + (
            "\nТы консультируешь клиента по вопросам питания, тренировок и образа жизни.\n"
            "Используй параметры и цель клиента для персонализированных рекомендаций."
        )
